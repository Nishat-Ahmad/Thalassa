    ...
    results.append(
        CheckResult(
            ...
            severity=(
                ...
                else ("warning" if missing_rate > 0.10 else "ok")
            ),
...
def _drift_checks(
    ...
    if metric.lower() != "ks":
        return [
            CheckResult(
                ...
                details={
                    "metric": metric,
                    "note": "Only 'ks' is supported; drift checks skipped.",
                },
            )
        ]
    ...
    severity = (
        "severe" if frac_severe >= 0.25 else ("warning" if frac_severe > 0.0 else "ok")
    )
    ...
    if not run_dir:
        results.append(
            CheckResult(
                ...
                severity="warning",
                details={
                    "note": "No registry run directory found; performance checks skipped."
                },
            )
        )
        return results
    ...
    if reg_meta is None:
        results.append(
            CheckResult(
                ...
                severity="warning",
                details={
                    "note": "Regressor artifacts not found; skipping regressor checks.",
                    "run_dir": run_dir,
                },
            )
        )
        ...
        if len(X) < 50 or y is None or len(y) < 50:
            results.append(
                CheckResult(
                    ...
                    severity="warning",
                    details={
                        "note": "Not enough labeled samples for regression eval.",
                        "samples": int(len(X)),
                    },
                )
            )
            ...
            results.append(
                CheckResult(
                    ...
                    severity="warning",
                    details={
                        "note": "xgboost not available in environment; cannot score regressor."
                    },
                )
            )
        else:
            ...
            if expected is not None and expected != int(X.shape[1]):
                results.append(
                    CheckResult(
                        ...
                        details={
                            ...
                            "provided_features": int(X.shape[1]),
                        },
                    )
                )
            else:
                ...
                y_test = y.iloc[split:]
                dtest = xgb.DMatrix(
                    X_test, feature_names=[str(c) for c in X_test.columns]
                )
    ...
    if cls_meta is None:
        results.append(
            CheckResult(
                ...
                severity="warning",
                details={
                    "note": "Classifier artifacts not found; skipping classifier checks.",
                    "run_dir": run_dir,
                },
            )
        )
        ...
        if y_cls is None:
            results.append(
                CheckResult(
                    ...
                    severity="warning",
                    details={
                        "note": "No target column for classification eval (need log_return or return)."
                    },
                )
            )
        else:
            ...
            if len(Xc) < 50:
                results.append(
                    CheckResult(
                        ...
                        severity="warning",
                        details={
                            "note": "Not enough samples for classification eval.",
                            "samples": int(len(Xc)),
                        },
                    )
                )
                ...
                results.append(
                    CheckResult(
                        ...
                        severity="warning",
                        details={
                            "note": "xgboost not available in environment; cannot score classifier."
                        },
                    )
                )
            else:
                ...
                if expected is not None and expected != int(Xc.shape[1]):
                    results.append(
                        CheckResult(
                            ...
                            details={
                                "expected_features": expected,
                                "provided_features": int(Xc.shape[1]),
                            },
                        )
                    )
                else:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        "--fail-on-severe",
        action="store_true",
        help="Exit non-zero if severe issues found",
    )
    parser.add_argument(
        "--feature-mode",
        default="engineered",
        choices=["engineered", "raw"],
        help="Used for reporting; affects drift windowing",
    )
    parser.add_argument(
        "--window", type=int, default=0, help="If set, evaluate drift on last N rows"
    )
    parser.add_argument(
        "--metric", default="ks", help="Drift metric (currently supports: ks)"
    )
    parser.add_argument(
        "--pvalue-threshold",
        type=float,
        default=0.001,
        help="p-value threshold for drift",
    )
    ...
    report = {
        ...
        "checks": [
            {"name": c.name, "severity": c.severity, "details": c.details}
            for c in checks
        ],
    }
