...
try:
    ...
    raise SystemExit(
        "scikit-learn is required for clustering. Install scikit-learn."
    ) from e
...
def fit_clusters(df: pd.DataFrame, n_clusters: int) -> tuple[dict, np.ndarray]:
    ...
    if len(X) < n_clusters:
        raise ValueError(
            "Insufficient rows to fit KMeans with the requested cluster count"
        )
...
def save_artifacts(
    meta: dict, labels: np.ndarray, registry_dir: str, ticker: str
) -> tuple[str, str]:
...
def main():
    parser = argparse.ArgumentParser(
        description="Train KMeans clusters from feature data"
    )
    parser.add_argument(
        "--features",
    ...
    print(
        json.dumps(
            {"status": "ok", "clusters_meta": meta_path, "cluster_labels": labels_path},
            indent=2,
        )
    )
