...
spec = importlib.util.spec_from_file_location(
    "train_pca", str(ROOT / "ml" / "train_pca.py")
)
train_pca = importlib.util.module_from_spec(spec)
spec.loader.exec_module(train_pca)

files = glob.glob(str(FEATURE_DIR / "*.parquet")) + glob.glob(
    str(FEATURE_DIR / "*.csv")
)
if not files:
...
for f in files:
    ...
    try:
        ...
        out_dir = os.path.join(
            REGISTRY_DIR, ticker, datetime.datetime.utcnow().strftime("%Y%m%d%H%M%S")
        )
        os.makedirs(out_dir, exist_ok=True)
        meta_path, comps_path = train_pca.save_artifacts(meta, comps, out_dir, ticker)
        # also save transformed info
        info = {
            "shape": list(comps.shape),
            "dtype": str(comps.dtype),
            "sample_min": float(np.nanmin(comps)),
            "sample_max": float(np.nanmax(comps)),
        }
